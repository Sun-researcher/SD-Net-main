...
class Dataset_Data(Dataset):
            ...
            try:
                ...
                if self.phase == 'train':
                    if np.random.rand() < 0.5:
                        img, img_mask, landmark, bbox = self.hflip(img, img_mask, landmark, bbox)

                img,img_mask,landmark, bbox, __ = crop_face(img, img_mask,landmark, bbox, margin=True, crop_by_bbox=False) #图像裁剪
                ...
                img, img_mask,_, __, ___, y0_new, y1_new, x0_new, x1_new = crop_face(img, img_mask,landmark, bbox, margin=False,
                ...
                img_mask = cv2.resize(img_mask, self.image_size, interpolation=cv2.INTER_LINEAR).astype('float32')
                img_mask = (img_mask > 0).astype('float32')
    ...
    def get_source_transforms(self):
        return alb.Compose([
            ...
            alb.OneOf([
                RandomDownScale(p=1),
                alb.IAASharpen(alpha=(0.2, 0.5), lightness=(0.5, 1.0), p=1),
            ], p=1),
    ...
    def get_transforms(self):
        return alb.Compose([
            ...
            alb.RGBShift((-20, 20), (-20, 20), (-20, 20), p=0.3),
            alb.HueSaturationValue(hue_shift_limit=(-0.3, 0.3), sat_shift_limit=(-0.3, 0.3),
                                   val_shift_limit=(-0.3, 0.3), p=0.3), #随机调整图像的色调、饱和度和亮度
    ...
    def collate_fn(self, batch):
        img,mask, label = zip(*batch)

        data = {}
        data['img'] = torch.tensor(img).float()
        data['label'] = torch.tensor(label).float()
...
if __name__ == '__main__':
    ...
    image_dataset = Dataset_Data(phase='train', image_size=256)
    batch_size = 64
    ...
    utils.save_image(img, 'FSBI.png', nrow=batch_size, normalize=False)
else:
    from .initialize import *
    from .funcs import IoUfrom2bboxes, crop_face, RandomDownScale
