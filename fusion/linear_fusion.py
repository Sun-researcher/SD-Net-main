...
from .activation_builder import buildActivation
from .normalization_builder import buildNormalization
...
class HdmProdBilinearFusion(nn.Module):
    ...
    def __init__(self, dim1, dim2,
        ...
        if bili_dropout is None: #不进行dropout
            self.Dropout = nn.Identity()
        else:
            self.Dropout = nn.Dropout(bili_dropout)
    ...
    def forward(self, features1, features2, **kwargs):
        ...
        #[b1,c1,h1,w1]->[b1,c1,h1*w1]->[b1,h1*w1,c1]->[b1*h1*w1,c1]
        features1 = features1.view(b1, c1, -1).permute(0, 2, 1).contiguous().view(-1, c1)
        #[b2, c2, h2, w2]->[b2,c2,h2*w2]->[b2,h2*w2,c2]->[b2*h2*w2,c2]
        ...
        #[b1,h1*w1,output_dim]->[b1,output_dim,h1*w1]->[b1,output_dim,h1,w1]
